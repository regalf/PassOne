...
ARGON2_PUBLIC char *hp_argon2_hash(uint8_t *key, uint32_t keylen, uint8_t *salt, uint32_t saltlen,
    ...
    if (type == 0) {
        ...
        // Patch locale: upstream 1.0.0+2 non gestisce type == 2 (argon2id) e
        // restituiva memoria non inizializzata -> KEK diversa tra setup e
        // login ("password mai valida"). Vendored in app/packages/argon2_ffi.
        PRINT_DEBUG("argon2id\n");
